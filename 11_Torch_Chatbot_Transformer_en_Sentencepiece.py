...
# 그 결과를 새로운 변수에 할당합니다.
is_within_len = (7 < train_df['src_len']) & (train_df['src_len'] <= 17) & (7 < train_df['trg_len']) & (train_df['trg_len'] <=17)
...
with open('corpus_src.txt', 'w', encoding='utf8') as f:
    f.write('\n'.join(dataset_df_8096['SRC']))

with open('corpus_trg.txt', 'w', encoding='utf8') as f:
    f.write('\n'.join(dataset_df_8096['TRG']))
...
train_df, test_df = train_test_split(dataset_df_8096, test_size=0.2)
